...
streams_list <- all_sheets %>% filter(str_detect(.$sheet_title, pattern = paste0(c("inventory", "Inventory"), collapse = "|"))) %>% group_by(sheet_title)

sheet_titles <- streams_list %>% filter(str_detect(sheet_title, pattern = "ry$")) %>% pull(sheet_title)
